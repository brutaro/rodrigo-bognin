...
try {
  const [row] = await sql`SELECT
    ...
    (SELECT volume_uuid IS NOT NULL AND quota_bytes = 9000000000 AND used_bytes >= 0 AND reserved_bytes >= 0 AND used_bytes + reserved_bytes <= quota_bytes FROM file_store_counter WHERE singleton) file_counter_valid,
    ...
    has_table_privilege('tria_app', 'file_version', 'insert') app_can_insert_file_version,
