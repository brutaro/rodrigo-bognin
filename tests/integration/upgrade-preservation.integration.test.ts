...
it('atualiza banco preenchido da migração 045 sem alterar dados; repetir é inócuo',async()=>{
 ...
 try{
  ...
  for(const file of files.filter(n=>Number(n.slice(0,3))<=45)){
  ...
  const snapshot=async()=>{
   ...
   for(const {tablename} of tables)result[tablename]=await sql!.unsafe(`SELECT to_jsonb(t) row FROM "${tablename}" t ORDER BY to_jsonb(t)::text`);
  ...
  migrate();expect(await snapshot()).toEqual(before);
  migrate();expect(await snapshot()).toEqual(before);
