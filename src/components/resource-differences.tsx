...
export function ResourceDifferences({ differences, busy, onResolve }: { differences: ResourceDifference[]; busy: boolean; onResolve: (decisions: ResourceDecisions) => void }) {
  ...
  return <div className="space-y-4 border-y border-[var(--border)] py-4">
    ...
    <p className="text-sm">Para substituir a base, use a planilha em todos os registros: os novos valores, inclusive zero, substituem os anteriores e os IDs ausentes saem da base vigente. O histórico é preservado. Você também pode decidir por registro.</p>
    <div className="flex flex-wrap gap-3 text-sm"><button type="button" disabled={busy} onClick={() => onResolve(Object.fromEntries(differences.map(row => [row.id,"incoming"])))} className="rounded border px-3 py-2">Substituir base pela planilha</button><button type="button" disabled={busy} onClick={() => setDecisions(Object.fromEntries(differences.map(row => [row.id,"keep"])))} className="rounded border px-3 py-2">Manter atuais em todos</button></div>
