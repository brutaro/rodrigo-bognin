...
type Note = FiscalNoteSummary & { adjustmentRequestId: string; restoreRequestId: string; deleteRequestId: string };
...
function DeleteNote({note,action}:{note:Note;action:Action}){
 const [state,submit,pending]=useActionState(action,initial);
 return <details className="mt-3 rounded-lg border border-red-200 p-3"><summary className="cursor-pointer font-semibold text-red-800">Excluir</summary><form action={submit} className="mt-3 space-y-3"><input type="hidden" name="fiscalNoteId" value={note.id}/><input type="hidden" name="expectedRevision" value={note.adjustmentRevision}/><input type="hidden" name="requestId" value={note.deleteRequestId}/><p className="text-xs">A nota deixará de compor os valores vigentes dos projetos e novos relatórios. Arquivos, histórico, pagamentos e publicações anteriores serão preservados.</p><label className="block text-xs font-bold">Motivo da exclusão<input name="reason" required minLength={3} maxLength={500} className="mt-1 h-10 w-full rounded-lg border px-2"/></label><button disabled={pending} className="rounded-lg bg-red-800 px-3 py-2 text-sm font-bold text-white">{pending?'Excluindo…':'Confirmar exclusão'}</button><Feedback state={state}/></form></details>;
}
