...
export function ProjectFiles({ projectId, documents, proofVersionIds = [] }: { projectId: string; documents: FileDocumentRecord[]; proofVersionIds?: string[] }) {
  ...
  return <section aria-labelledby="vault-title" className="rounded-2xl border border-[var(--border)] bg-white shadow-sm">
    ...
    <div className="divide-y divide-[var(--border)]">
      {documents.map((document) => <article key={document.id} className="p-5">
        {document.versions.some(version=>proofVersionIds.includes(version.id)) && <p className="mb-2 text-xs font-semibold text-[var(--brand)]">A versão vinculada como comprovante acompanha a publicação.</p>}
