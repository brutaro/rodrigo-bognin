...
export function ResourceImport({ project }: { project?: { id: string; title: string } }) {
  ...
  async function request(payload: object) {
    const response = await fetch("/api/sources/resources", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, includeActivities, overwrite, ...(project ? { projectId: project.id } : {}) }) });
  ...
  return <div className="space-y-6">
    ...
    {!project && <label className="flex gap-3 rounded-lg border border-[var(--border)] bg-white p-4 text-sm"><input type="checkbox" checked={overwrite} disabled={busy} onChange={e=>{setOverwrite(e.target.checked);setPreview(undefined);setConfirmed(false);}} /><span><strong>Sobrescrever a base anterior</strong><span className="block">Retira os registros ausentes e arquiva os projetos da base anterior que não aparecem nesta planilha. Sem marcar, os registros e projetos ausentes são mantidos. Projetos novos serão criados na confirmação.</span></span></label>}
    ...
    {preview && <section className="rounded-lg border border-[var(--border)] bg-white p-6 space-y-4">
      ...
      <p className="text-sm">Valor e Horas vazios nas colunas mapeadas são importados como zero, inclusive quando a fórmula retorna vazio. Fórmulas com erro ou sem resultado salvo precisam ser corrigidas.</p>
      {preview.projects && <div className="rounded-lg bg-slate-50 p-4 text-sm"><p className="font-semibold">Projetos: {preview.projects.create.length} novos · {preview.projects.archive.length} serão retirados da lista ativa · {preview.projects.restore.length} reativados</p>{preview.projects.create.length>0 && <p>Criar: {preview.projects.create.map(p=>p.title).join(", ")}</p>}{preview.projects.archive.length>0 && <p>Arquivar: {preview.projects.archive.map(p=>p.title).join(", ")}. Histórico, arquivos e vínculos serão preservados.</p>}</div>}
