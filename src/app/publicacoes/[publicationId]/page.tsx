    ...
    <AppShell>
      <main className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-10">
        ...
        <div className="mt-6 grid gap-6">
          ...
          {"files" in view ? <section aria-labelledby="published-files" className="rounded-2xl border border-[var(--border)] bg-white shadow-sm"><div className="border-b border-[var(--border)] p-5"><h2 id="published-files" className="text-xl font-bold">Arquivos vinculados</h2><p className="mt-1 text-sm text-slate-500">Downloads autenticados da versão exata conferida nesta publicação.</p></div><ul className="divide-y divide-[var(--border)]">{view.files.map((file) => <li key={file.versionId} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><strong>{file.title}</strong><p className="mt-1 text-xs text-slate-500">V{file.version} · {file.originalName} · SHA-256 {file.sha256}</p></div><a href={`/api/files/${file.versionId}/download`} className="font-bold text-[var(--brand)] hover:underline">Baixar arquivo</a></li>)}</ul>{!view.files.length ? <p className="p-5 text-sm text-slate-500">Nenhum arquivo vinculado.</p> : null}</section> : null}
        </div>
