...
export default async function Home() {
  ...
  const currentProject = projects.filter(project => !project.archived && project.lastOpenedAt).sort((a,b) => Date.parse(b.lastOpenedAt!) - Date.parse(a.lastOpenedAt!))[0];
    ...
    <AppShell>
      <main className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-10">
        <section className="mb-7 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link href="/projetos/novo" className="rounded-lg bg-[var(--brand)] px-4 py-3 text-sm font-semibold text-white">Novo projeto</Link>
        </section>
        ...
        {currentProject ? (
          <section aria-labelledby="continue-title" className="mb-7 rounded-2xl border border-blue-200 bg-blue-50 p-5 sm:p-6">
            <p id="continue-title" className="text-xs font-bold uppercase tracking-[0.16em] text-blue-800">Continuar de onde parou</p>
        ...
        <p className="mt-6 text-sm text-[var(--ink-muted)]">
          <Link href="/publicacoes" className="underline">Consultar publicações anteriores</Link>
        </p>
      </main>
    </AppShell>
  );
