...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("prepare"), sourceId: z.uuid(), ordinal: z.number().int().min(0).max(31), includeActivities: z.boolean().optional(), overwrite: z.boolean().optional(), projectId: z.string().min(1).max(120).optional(), mapping: z.record(z.string(), z.number().int().min(-1).max(255)), delimiter: z.enum([",", ";", "\t"]).optional() }),
