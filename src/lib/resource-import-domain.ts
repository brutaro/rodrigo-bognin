...
export type ResourceRow = { id: string; project: string; date: string; activity: string; amount: string; hours: string; nature: string };
...
export function validateResourceRows(rows: string[][], mapping: ResourceMapping, width: number, locators?:string[]) {
  ...
  for (const key of ["id", "project", "date", "activity", "amount"]) if (!(mapping[key] >= 0)) throw new Error("Mapeie ID, Projeto, Data, Atividade e Valor.");
  rows.forEach((row, index) => {
    if (row.every(cell => !cell.trim())) return;
    try {
      const value = (key: string) => (row[mapping[key]] ?? "").trim();
      ...
      const result = { id, project: value("project"), date, activity: value("activity"), amount: decimal(value("amount")), hours: value("hours") ? decimal(value("hours")) : "", nature: value("nature") };
