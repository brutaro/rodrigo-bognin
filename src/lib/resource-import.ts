...
import { resourceDifferences, resolveResourceRows, type ResourceDecisions, resourceTotalCents, compareResources, suggestResourceMapping, validateResourceRows, type ResourceRow, type ResourceMapping } from "./resource-import-domain";
...
export async function prepareResources(sourceId: string, ordinal: number, mapping: ResourceMapping, delimiter?: "," | ";" | "\t") {
  const doc = await sourceDocument(sourceId, delimiter);
  const sheet = doc.sheets.find(s => s.selection.ordinal === ordinal);
  if (!sheet) throw new Error("Selecione uma aba válida.");
  const data = validateResourceRows(sheet.rows, mapping, sheet.headers.length,sheet.locators);
  const current = await currentResources();
  const id = randomUUID();
  const hash = createHash("sha256").update(JSON.stringify({ source: doc.hash, ordinal, mapping, rows: data.rows })).digest("hex");
  await getSql()`INSERT INTO resource_import(id,source_file_id,content_hash,sheet_name,rows,errors,base_id)
    VALUES (${id},${sourceId},${hash},${sheet.selection.name},${getSql().json(data.rows)},${getSql().json(data.errors)},${current?.id ?? null})`;
  return { id, hash, differences: resourceDifferences(current?.rows ?? [],data.rows), count: data.rows.length, errors: data.errors.slice(0, 30), errorCount: data.errors.length, sample: data.rows.slice(0, 20), totalCents: resourceTotalCents(data.rows).toString(), ...compareResources(current?.rows ?? [], data.rows) };
}
export async function applyResources(id: string, hash: string) {
  ...
  return getSql().begin(async tx => {
    ...
    if (!comparison.added && !comparison.changed && !comparison.absent) return { id: current.import_id, reused: true };
    ...
    // Edição posterior à publicação exige uma nova conferência dos projetos afetados.
    await tx`UPDATE project_draft SET revision=revision+1,updated_at=now() WHERE project_id IN
      (SELECT p.id FROM project p WHERE coalesce(nullif(p.resource_source_title,''),p.title) IN (SELECT value->>'project' FROM jsonb_array_elements(${tx.json(preview.rows)}))
       OR coalesce(nullif(p.resource_source_title,''),p.title) IN (SELECT value->>'project' FROM resource_import previous CROSS JOIN LATERAL jsonb_array_elements(previous.rows) WHERE previous.id=${current?.import_id ?? null}))`;
...
export async function resolveResources(id: string, hash: string, decisions: ResourceDecisions) {
  ...
  const resolvedHash = createHash("sha256").update(JSON.stringify({parent:hash,rows,decisions})).digest("hex");
  await getSql()`INSERT INTO resource_import(id,source_file_id,content_hash,sheet_name,rows,errors,base_id,resolved_from,decisions)
    VALUES (${resolvedId},${preview.source_file_id},${resolvedHash},${preview.sheet_name},${getSql().json(rows)},'[]',${preview.base_id},${id},${getSql().json(decisions)})`;
  return {id:resolvedId,hash:resolvedHash,count:rows.length,errors:[],errorCount:0,sample:rows.slice(0,20),totalCents:resourceTotalCents(rows).toString(),differences:[],resolved:true,...compareResources(current?.rows ?? [],rows)};
}
