...
export type PublicPublicationV3 = Omit<PublicPublicationV2, "schemaVersion" | "rendererVersion"> & {
  schemaVersion: "tria-publication-v3";
  rendererVersion: "tria-export-v3";
  files: Array<{ code: string; documentId: string; versionId: string; title: string; version: number;
    originalName: string; mediaType: string; sizeBytes: number; sha256: string }>;
};
...
export function toPublicPublicationV3(publication: DemoPublication): PublicPublicationV3 {
  if (publication.schemaVersion !== "tria-publication-v3" || publication.rendererVersion !== "tria-export-v3") {
    throw new Error("Snapshot incompatível com o renderizador tria-export-v3.");
  }
  const base = toPublicPublicationV2({ ...publication, schemaVersion: "tria-publication-v2", rendererVersion: "tria-export-v2" });
  return {
    ...base,
    schemaVersion: "tria-publication-v3",
    rendererVersion: "tria-export-v3",
    files: (publication.files ?? []).map((file, index) => ({
      code: `ARQ-${String(index + 1).padStart(3, "0")}`,
      documentId: file.documentId,
      versionId: file.versionId,
      title: sanitizePublicText(file.title),
      version: file.version,
      originalName: sanitizePublicText(file.originalName),
      mediaType: sanitizePublicText(file.mediaType),
...
function buildPublicationCsvView(view: PublicPublicationV2 | PublicPublicationV3) {
...
function buildPublicationCsvV2(publication: DemoPublication) {
  return buildPublicationCsvView(toPublicPublicationV2(publication));
}

function buildPublicationHtmlView(view: PublicPublicationV2 | PublicPublicationV3) {
...
function buildPublicationHtmlV2(publication: DemoPublication) {
  return buildPublicationHtmlView(toPublicPublicationV2(publication));
}

function buildPublicationCsvV3(publication: DemoPublication) {
  const view = toPublicPublicationV3(publication);
  const legacy = buildPublicationCsvView(view);
  const rows = view.files.map((file) => [view.publicationCode, "file_metadata", file.code, file.documentId,
    file.title, `${file.mediaType} · versão ${file.version}`, String(file.sizeBytes), null, file.originalName,
    null, null, null, null, "Cofre pessoal verificado", null, null, `SHA-256 ${file.sha256}`]
    .map(safeCsvCell).join(";"));
  return rows.length ? `${legacy.slice(0, -2)}${rows.map((row) => `\r\n${row}`).join("")}\r\n` : legacy;
}

function buildPublicationHtmlV3(publication: DemoPublication) {
  const view = toPublicPublicationV3(publication);
  const files = view.files.map((file) => `<tr><td>${escapeHtml(file.code)}</td><td>${escapeHtml(file.title)}</td><td>V${escapeHtml(file.version)}</td><td>${escapeHtml(file.originalName)}</td><td>${escapeHtml(file.sizeBytes)}</td><td><code>${escapeHtml(file.sha256)}</code></td></tr>`).join("");
  const section = `<section class="card"><h2>Arquivos vinculados</h2><p>Somente metadados fazem parte deste HTML. Os bytes permanecem no cofre autenticado.</p><div class="table-wrap"><table><thead><tr><th>Código</th><th>Título</th><th>Versão</th><th>Nome original</th><th>Bytes</th><th>SHA-256</th></tr></thead><tbody>${files || '<tr><td colspan="6">Nenhum arquivo vinculado.</td></tr>'}</tbody></table></div></section>`;
  return buildPublicationHtmlView(view)
    .replace('<footer class="footer">', `${section}<footer class="footer">`);
}
